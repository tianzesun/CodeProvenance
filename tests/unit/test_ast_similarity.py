...
import pytest
from src.backend.core.similarity.ast_similarity import (
    ASTSimilarity,
    ASTNode,
    TreeEditDistance,
    ControlFlowGraph,
...
class TestASTNode:
    ...
    def test_to_tuple(self):
        ...
        node = ASTNode(
            "ROOT", "", children=[ASTNode("LEAF", "a"), ASTNode("LEAF", "b")]
        )
    ...
    def test_subtree_size(self):
        ...
        node = ASTNode(
            "ROOT",
            "",
            children=[ASTNode("A", "", children=[ASTNode("B", "")]), ASTNode("C", "")],
        )
...
class TestTreeEditDistance:
    ...
    def test_different_trees(self):
        ...
        tree_a = ASTNode("ROOT", "", children=[ASTNode("IDENTIFIER", "a")])
        tree_b = ASTNode("ROOT", "", children=[ASTNode("IDENTIFIER", "b")])
...
class TestASTSimilarity:
    ...
    def test_for_and_while_normalize_as_iterative_blocks(self):
        """Test loop syntax rewrites still match structurally."""
        tokens_a = [
        ...
        parsed_a = {"tokens": tokens_a, "raw": "for i < n return i"}
        parsed_b = {"tokens": tokens_b, "raw": "while j < n return j"}

        score = self.algorithm.compare(parsed_a, parsed_b)
        assert score > 0.8

    def test_if_and_switch_normalize_as_decision_blocks(self):
        """Test decision syntax rewrites keep meaningful AST similarity."""
        tokens_a = [
            {"type": "KEYWORD", "value": "if"},
            {"type": "VARIABLE", "value": "status"},
            {"type": "OPERATOR", "value": "=="},
            {"type": "NUMBER", "value": "1"},
            {"type": "KEYWORD", "value": "return"},
        ]
        tokens_b = [
            ...
            {"type": "VARIABLE", "value": "state"},
            {"type": "KEYWORD", "value": "case"},
            {"type": "NUMBER", "value": "1"},
            {"type": "KEYWORD", "value": "return"},
        ]

        parsed_a = {"tokens": tokens_a, "raw": "if status == 1 return status"}
        parsed_b = {"tokens": tokens_b, "raw": "switch state case 1 return state"}
