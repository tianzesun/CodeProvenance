...
class TestAuthMiddleware:
    ...
    def test_invalid_session_cookie_detected(self, monkeypatch):
        ...
        assert (
            self.middleware._has_valid_session_cookie(
                _make_request(cookie_value=tampered)
            )
            is False
        )
        assert (
            self.middleware._has_valid_session_cookie(
                _make_request(cookie_value="not-a-jwt")
            )
            is False
        )
    ...
    def test_exclude_paths(self):
        """Public paths are limited to health and versioned auth endpoints."""
        exclude_paths = ["/", "/health", "/api/v1/health", "/api/v1/auth"]

        for path in exclude_paths:
            assert path in self.middleware.exclude_paths

        for path in ("/docs", "/redoc", "/openapi.json"):
            assert path not in self.middleware.exclude_paths
...
class TestRateLimitMiddleware:
    ...
    def setup_method(self):
        ...
        self.middleware = RateLimitMiddleware(
            self.app, redis_url="redis://localhost:6379"
        )
    ...
    def test_tenant_rate_limits(self):
        ...
        rate_limits = {"free": 10, "basic": 60, "pro": 300, "enterprise": 1000}
...
class TestErrorHandling:
    ...
    def test_authentication_error(self):
        ...
        error = HTTPException(status_code=401, detail="Invalid or inactive API key")
    ...
    def test_validation_error(self):
        ...
        error = HTTPException(status_code=400, detail="No submissions provided")
