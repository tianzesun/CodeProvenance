...
Combines AST alignment + GST alignment into forensic proof reports.

This turns the black-box score generator into a forensic tool that answers:
"Where exactly is the plagiarism, and how was it transformed?"
"""
...
class ExplainableReport:
    ...
    def to_string(self) -> str:
        ...
        lines.append("=" * 48)
        lines.append("")
        lines.append(f"Overall similarity: {self.overall_score:.2f} ({self._confidence_label()})")
        lines.append("")
        ...
        for i, block in enumerate(self.gst_blocks, 1):
            ...
            lines.append(f"  A[{block['a_start']}-{block['a_end']}] ↔ B[{block['b_start']}-{block['b_end']}]")
...
class GSTAligner:
    ...
    def align(self, tokens_a: List[str], tokens_b: List[str], min_match: int = 3) -> List[Tuple[int, int, int]]:
        ...
        while True:
            ...
            for i in range(len(tokens_a)):
                ...
                for j in range(len(tokens_b)):
                    ...
                    l = 0
                    while (i + l < len(tokens_a) and
                           j + l < len(tokens_b) and
                           not marked_a[i + l] and
                           not marked_b[j + l] and
                           tokens_a[i + l] == tokens_b[j + l]):
                        l += 1
...
class ReportGenerator:
    ...
    def generate_report(self,
                        ...
                        final_score: float) -> ExplainableReport:
        ...
        if ast_valid:
            ...
            ast_matches = [
                ...
                for a, b in matches
            ][:15]  # Limit to top 15 matches
        
        # GST alignment
        from src.engines.similarity.token_similarity import TokenSimilarity
        tokenizer = TokenSimilarity()
        tokens_a = tokenizer._normalize_identifiers(tokenizer._extract_tokens({"raw": code_a}))
        tokens_b = tokenizer._normalize_identifiers(tokenizer._extract_tokens({"raw": code_b}))
        ...
        if ast_score > 0.7 and abs(ast_score - token_score) > 0.2:
            ...
            transformation_analysis.append("Code structure preserved, surface changes only")
        ...
        confidence = min(1.0, final_score + 0.1 * (len(ast_matches) / 10) + 0.1 * (len(gst_blocks) / 5))
