...
function getApiErrorMessage(error) {
  return error?.response?.data?.error || error?.response?.data?.detail || error?.message || 'AI detection failed';
}
...
export default function AIDetectorPage() {
  ...
  const runDetection = async () => {
    ...
    fd.append('course_name', courseName || 'AI Detector');
    fd.append('assignment_name', assignmentName || 'AI Generated Code Review');
        ...
        <div className="space-y-8">
          <section className="theme-card-strong rounded-[30px] overflow-hidden">
            <div className="theme-section-line px-6 py-5 lg:px-7">
              <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--text-secondary)]">
                      Upload one assignment or a batch of submissions. IntegrityDesk returns explainable AI-generated-code signals for professor review.
                    </p>
                  </div>
                </div>
                <button
                ...
                >
                  {uploading ? <><Loader2 size={16} className="animate-spin" />Detecting...</> : <><Shield size={16} />Run AI Detector<ArrowRight size={15} /></>}
                </button>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            {files.length === 0 ? (
              ...
              >
                ...
                <div className="mt-5 text-base font-semibold text-slate-900">Upload code files or a ZIP archive</div>
                <div className="mt-2 max-w-md text-sm leading-6 text-slate-500">
                  AI Detector supports one file for a single-student review or many files for batch screening.
                </div>
              ...
              <div className="p-5">
                <div className="flex items-center justify-between gap-4">
                  <div className="text-sm font-semibold text-slate-900">{files.length} file{files.length === 1 ? '' : 's'} ready</div>
          ...
          <section className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <div className="flex items-center justify-between gap-4 border-b border-slate-100 px-5 py-4">
              <div>
                <div className="text-sm font-semibold text-slate-900">AI Detector History</div>
                <div className="mt-1 text-xs text-slate-500">Recent reports stay available for review.</div>
              </div>
              <CalendarClock size={18} className="text-slate-400" />
            </div>
            {history.length === 0 ? (
              <div className="px-5 py-8 text-sm text-slate-500">No AI Detector reports yet.</div>
            ) : (
              <div className="divide-y divide-slate-100">
                {history.map((job) => (
                  ...
                  >
                    <div>
                      <div className="font-medium text-slate-900">{job.assignment_name || 'AI Detector Report'}</div>
                      <div className="mt-1 text-xs text-slate-500">{job.course_name || 'Course'} · {job.file_count || 0} file{job.file_count === 1 ? '' : 's'}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${getTone(job.summary?.highest_ai_probability || 0)}`}>
                        {Math.round((job.summary?.highest_ai_probability || 0) * 100)}% highest
                      </span>
