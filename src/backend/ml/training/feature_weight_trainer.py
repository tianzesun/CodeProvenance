...
class FeatureWeightTrainer:
    ...
    def train(self, X: list[list[float]], y: list[int]) -> dict[str, Any]:
        ...
        return {
            ...
            "feature_importance": dict(
                zip(FEATURE_ORDER, self._model.coef_[0].tolist())
            ),
        }
    ...
    def predict_proba(self, X: list[list[float]]) -> list[float]:
        ...
        return self._model.predict_proba(
            self._scale(X, self._scaler_mean, self._scaler_std)
        )[:, 1].tolist()
    ...
    def _scale(X, m=None, s=None):
        if m is None:
            ...
            s = [
                max((sum((X[i][j] - m[j]) ** 2 for i in range(n)) / n) ** 0.5, 1e-8)
                for j in range(m2)
            ]
            return (
                [[(X[i][j] - m[j]) / s[j] for j in range(m2)] for i in range(n)],
                m,
                s,
            )
    ...
    def _train_simple(X, y):
        ...
        for _ in range(100):
            for xi, yi in zip(X, y):
                p = sum(we * x for we, x in zip(w, xi)) + b
                e = yi - p
                w = [we + lr * e * x for we, x in zip(w, xi)]
                b += lr * e
        return {
            "weights": dict(zip(FEATURE_ORDER, w)),
            "metrics": {"note": "simple training"},
            "feature_importance": dict(zip(FEATURE_ORDER, w)),
            "intercept": b,
        }
