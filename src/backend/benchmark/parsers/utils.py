...
def parse_tool_output(tool_name: str, input_path: Path, dataset: str = "",
    ...
    if tool_name == "jplag":
        from src.backend.benchmark.parsers.jplag_parser import JPlagParser
        return JPlagParser(dataset=dataset, threshold=threshold).parse(input_path)
    elif tool_name == "moss":
        from src.backend.benchmark.parsers.moss_parser import MossParser
        return MossParser(dataset=dataset, threshold=threshold).parse(input_path)
    elif tool_name == "nicad":
        from src.backend.benchmark.parsers.nicad_parser import NiCadParser
