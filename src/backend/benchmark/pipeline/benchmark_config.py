...
class MetricsConfig:
    ...
    metrics: List[str] = field(
        default_factory=lambda: ["precision", "recall", "f1", "accuracy", "map", "mrr"]
    )
    ...
    def config_hash(self) -> str:
        ...
        config_dict = {
            ...
            "parser": {
                ...
                "max_ast_depth": self.parser.max_ast_depth,
            },
            "evaluation": {"mode": self.evaluation.mode},
            "metrics": self.metrics.metrics,
            "threshold": {
                "optimize": self.threshold.optimize,
                "strategy": self.threshold.strategy,
            },
            "output": {
                "json": self.output.json,
                "html": self.output.html,
                "leaderboard": self.output.leaderboard,
            },
        }
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "BenchmarkConfig":
        ...
        return cls(
            ...
            metrics=MetricsConfig(
                metrics=d.get("metrics", {}).get(
                    "metrics", ["precision", "recall", "f1", "map", "mrr"]
                )
            ),
