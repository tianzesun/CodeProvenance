...
class MetricsStage(PipelineStage):
    ...
    def execute(
        ...
        from src.backend.benchmark.evaluation.metrics import precision, recall, f1_score, accuracy
...
class ReportingStage(PipelineStage):
    ...
    def execute(
        ...
        if config.get("output", {}).get("json", True):
            ...
            from src.backend.benchmark.reporting import JSONReportWriter
        ...
        if config.get("output", {}).get("html", False):
            ...
            from src.backend.benchmark.reporting import HTMLReportWriter
        ...
        if config.get("output", {}).get("leaderboard", False):
            ...
            from src.backend.benchmark.reporting import Leaderboard, LeaderboardEntry
