...
from src.backend.evalforge.core import CloneType, CodePair, Transformer

project_root = Path(__file__).parent.parent.parent.parent
sys.path.insert(0, str(project_root))
from src.backend.benchmark.generators.utils.rename_utils import rename_identifiers
from src.backend.benchmark.generators.utils.transform_utils import (
    add_dead_code,
    convert_loops,
    reorder_statements,
)
...
class Dataset:
    ...
    def load(cls, path: Path) -> Dataset:
        ...
        return cls(name=data["name"], pairs=pairs, metadata=data.get("metadata", {}))
    ...
    def apply_transformations(
        self, transformations: list[str], seed: int = 42
    ) -> Dataset:
    ...
    def sample_balanced(
        self, positive_ratio: float = 0.5, max_pairs: int | None = None, seed: int = 42
    ) -> Dataset:
    ...
    def stratified_split(
        ...
        return (
            Dataset(
                name=f"{self.name}_train", pairs=train_pairs, metadata=self.metadata
            ),
...
def load_poj104() -> Dataset:
    ...
    Path("/home/tsun/CodeProvenance/benchmark/data/poj104")
...
def load_bigclonebench() -> Dataset:
    ...
    Path("/home/tsun/CodeProvenance/benchmark/data/bigclonebench")
    return Dataset(name="bigclonebench", pairs=[])


def load_codesearchnet(language: str = "python") -> Dataset:
    """Load CodeSearchNet dataset."""
    Path(f"/home/tsun/CodeProvenance/benchmark/data/codesearchnet_{language}")
...
def load_codexglue_clone() -> Dataset:
    ...
    Path("/home/tsun/CodeProvenance/benchmark/data/codexglue_clone")
