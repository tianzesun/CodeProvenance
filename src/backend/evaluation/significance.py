...
class EngineComparisonResult:
    ...
    is_significant: bool
    interpretation: (
        str  # "A is significantly better", "No significant difference", etc.
    )
...
def compare_engines(
    ...
    rel_diff = (
        (abs_diff / max(value_a, value_b) * 100) if max(value_a, value_b) > 0 else 0.0
    )
    ...
    if mc.is_significant:
        interpretation = (
            ...
            f"p={mc.p_value:.4f})"
            if better_a
            else f"Engine B is significantly better ({metric}: {value_b:.4f} vs {value_a:.4f}, "
            f"p={mc.p_value:.4f})"
        )
    else:
        # Check if CI overlap
        overlap = not (
            ci_a[metric]["ci_lower"] > ci_b[metric]["ci_upper"]
            or ci_b[metric]["ci_lower"] > ci_a[metric]["ci_upper"]
        )
        interpretation = (
