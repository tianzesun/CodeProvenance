...
class ReproducibilityReport:
    ...
    def generate(self):
        ...
        self.report_data["hashes"]["results_hash"] = hashlib.sha256(
            results_str.encode()
        ).hexdigest()
...
class CIGate:
    ...
    def check_performance(
        self, current: EvaluationResult, min_f1: float = 0.8, max_ece: float = 0.1
    ) -> bool:
        """
        ...
        if current.f1 < min_f1:
            logger.error(
                f"Performance Check Failed: F1 {current.f1:.4f} < {min_f1:.4f}"
            )
            passed = False

        if current.ece and current.ece > max_ece:
            logger.error(
                f"Performance Check Failed: ECE {current.ece:.4f} > {max_ece:.4f}"
            )
            passed = False
    ...
    def check_regression(
        self, current: EvaluationResult, tolerance: float = 0.01
    ) -> bool:
        """
        ...
        if current.f1 < baseline_f1 - tolerance:
            logger.error(
                f"Regression Check Failed: F1 {current.f1:.4f} < Baseline {baseline_f1:.4f} - {tolerance}"
            )
            return False
        return True
