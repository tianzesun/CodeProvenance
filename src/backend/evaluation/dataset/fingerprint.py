...
class ContaminationChecker:
    ...
    def __init__(self, fingerprinter: DatasetFingerprinter | None = None):
    ...
    def add_dataset(self, name: str, directory_path: Path):
        ...
        self.dataset_fingerprints[name] = self.fingerprinter.fingerprint_directory(
            directory_path
        )
            ...
            for j in range(i + 1, len(dataset_names)):
                ...
                if common_hashes:
                    ...
                    for h in common_hashes:
                        ...
                        overlaps[key].append(
                            {"hash": h, "ds1_files": files1, "ds2_files": files2}
                        )
        return overlaps

    def get_deduplicated_dataset(
        self, dataset_path: Path, reference_datasets: list[Path]
    ) -> list[Path]:
        """
