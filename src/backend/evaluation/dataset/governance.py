...
from .fingerprint import ContaminationChecker, DatasetFingerprinter
...
class DatasetGovernance:
    ...
    def register_dataset(self, name: str, path: Path, version: str, description: str):
        ...
        self.metadata["datasets"][name] = {
            ...
            "content_hash": self.fingerprinter.compute_content_hash(
        ...
        if overlaps:
            logger.warning(
                f"Contamination detected for dataset {name}: {overlaps.keys()}"
            )
    ...
    def validate_all(self):
        ...
        for name, info in self.metadata["datasets"].items():
            ...
            current_hash = self.fingerprinter.compute_content_hash(
                json.dumps(current_f, sort_keys=True)
            )

            if current_hash != info["content_hash"]:
                logger.error(
                    f"Dataset {name} content changed! Expected: {info['content_hash']}, Got: {current_hash}"
                )
