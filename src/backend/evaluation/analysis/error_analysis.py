...
class ErrorAnalyzer:
    ...
    def analyze(
        self, predictions: list[tuple[str, str, float, bool]]
    ) -> ErrorAnalysisResult:
        errors = []
        fp = fn = 0
        ...
        if fp > fn:
            recs.append(
                "High FP: consider increasing threshold or improving filtering."
            )
        elif fn > fp:
            recs.append(
                "High FN: consider decreasing threshold or enabling semantic analysis."
            )
        if fp + fn > 10:
                ...
                [
                    {
                        "file_a": e.file_a,
                        "file_b": e.file_b,
                        "label": 1 if e.actual_label else 0,
