...
class ThresholdStabilityReport:
    ...
    def summary(self) -> str:
        ...
        if self.robustness_score >= 0.9:
            lines.append(
                "  >>> Model is HIGHLY ROBUST - threshold choice is not critical"
            )
        elif self.robustness_score >= 0.7:
            lines.append(
                "  >>> Model is MODERATELY ROBUST - threshold choice matters somewhat"
            )
        elif self.robustness_score >= 0.5:
            lines.append("  >>> Model is SENSITIVE - threshold choice is important")
        else:
            lines.append(
                "  >>> Model is FRAGILE - small threshold changes cause large F1 swings"
            )
...
class FailureClusterReport:
    ...
    def summary(self) -> str:
        ...
        for surface, count in sorted(
            self.attack_surfaces.items(), key=lambda x: x[1], reverse=True
        ):
...
class ThresholdStabilityAnalyzer:
    ...
    def analyze(
        ...
        if working_starts:
            report.working_range = (
                working_starts[0],
                working_ends[-1] if working_ends else 1.0,
            )
            report.working_range_width = (
                report.working_range[1] - report.working_range[0]
            )
...
class FailureClusterAnalyzer:
    ...
    def cluster(
        ...
        for i, (score, label, clone_type, code_a, code_b) in enumerate(self.results):
            ...
            if predicted != label:
                comp_scores = (
                    self.component_scores[i] if i < len(self.component_scores) else {}
                )
                failures.append(
                    {
                        "index": i,
        ...
        for pattern_name, pattern_failures in sorted(
            ...
            avg_error = sum(f["error"] for f in pattern_failures) / len(
                pattern_failures
            )
