...
Takes EAM output and produces actionable optimization targets ranked
by expected impact on overall detection quality.

Usage:
    from benchmark.analysis.causal_ranking import CausalRankingEngine

    engine = CausalRankingEngine(eam_report)
...
class CausalRankingEngine:
    ...
    Converts attribution data into a prioritized improvement roadmap.

    Usage:
        causal = CausalRankingEngine(eam_report, stability_report, clustering_report)
    ...
    def rank_improvements(self) -> CausalRankingReport:
        ...
        # 1. From EAM: Component-based improvements
        if self.eam_report:
            candidates.extend(self._rank_from_eam())
    ...
    def _rank_from_eam(self) -> list[ImprovementCandidate]:
        """Generate candidates from EAM component losses.
        ...
        if not self.eam_report:
            return candidates

        # Primary causes from EAM
        causes = getattr(self.eam_report, "primary_cause_distribution", {})
    ...
    def _rank_from_clustering(self) -> list[ImprovementCandidate]:
        ...
        for cluster in clusters:
            if (
                getattr(cluster, "recommended_fix", "")
                and getattr(cluster, "size", 0) > 5
            ):
                ...
                combined.append(
                    ImprovementCandidate(
                        ...
                        implementation_complexity=min(
                            cands,
                            key=lambda c: {"easy": 0, "medium": 1, "hard": 2}.get(
                                c.implementation_complexity, 1
                            ),
    ...
    def _map_cause_to_improvement(self, cause_name: str) -> dict[str, Any] | None:
