...
class AttributionReport:
    ...
    component_effectiveness: dict[str, ComponentEffectiveness] = field(
    ...
    def summary(self) -> str:
        ...
        for comp, loss in sorted(
            self.component_losses.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        for cause, count in sorted(
            self.primary_cause_distribution.items(), key=lambda x: x[1], reverse=True
        ):
...
class ErrorAttributionModel:
    ...
    def __init__(
        self,
        component_engines: dict[str, Callable[[str, str], float]] | None = None,
    ...
    def _default_components(self) -> dict[str, Callable[[str, str], float]]:
    ...
    def analyze(
        ...
        for pair in pairs:
            ...
            predicted = (
                main_engine.compare(pair.code_a, pair.code_b) if main_engine else 0.0
            )
            ...
            for comp_name, comp_func in self.component_engines.items():
                ...
                component_gt[comp_name].append(
                    (component_scores[comp_name], ground_truth)
                )
        ...
        if total_error > 0:
            for comp in self.component_engines:
                total_loss = sum(
                    a.loss_contribution.get(comp, 0.0) for a in all_attributions
                )
        ...
        for clone_type, attributions in type_attributions.items():
            ...
            for comp in self.component_engines:
                comp_loss = sum(
                    a.loss_contribution.get(comp, 0.0) for a in attributions
                )
        ...
        for comp, scores in component_gt.items():
            if scores:
                report.component_effectiveness[comp] = (
                    self._compute_component_effectiveness(comp, scores)
                )
    ...
    def _decompose_loss(
        ...
        total_weight = sum(
            self.engine_weights.get(c, 1.0) for c in self.component_engines
        )
    ...
    def _compute_component_effectiveness(
        ...
        numerator = sum(
            (c - mean_comp) * (g - mean_gt) for c, g in zip(comp_vals, gt_vals)
        )
...
def _structural_similarity(code_a: str, code_b: str) -> float:
    ...
    def extract_features(code: str) -> dict[str, float]:
        ...
        features["avg_indent"] = (
            sum(indent_levels) / len(indent_levels) if indent_levels else 0
        )
