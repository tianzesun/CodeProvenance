...
class GitAnalysisResult:
    ...
    file_authors: dict[str, dict[str, int]] = field(
        default_factory=dict
    )  # file -> author -> count
    suspicious_patterns: list[dict[str, Any]] = field(default_factory=list)
    original_authorship: dict[str, str] = field(
        default_factory=dict
...
class GitAnalyzer:
    ...
    def find_original_author(self, file_path: str) -> str | None:
        """
    ...
    def detect_copied_code(
        self, local_code: str, repo_path: str
    ) -> list[dict[str, Any]]:
        """
    ...
    def _hash_code_chunks(
        self, code: str, chunk_size: int = 20
    ) -> list[tuple[str, str]]:
    ...
    def _detect_suspicious_patterns(
        self, author_commits: dict[str, int], file_authors: dict[str, dict[str, int]]
    ) -> list[dict[str, Any]]:
...
class GitBlameVisualizer:
    ...
    def generate_blame_html(
        ...
        highlighted_authors: list[str] | None = None,
    ) -> str:
        """
...
"""
    ...
    def _generate_author_colors(self, entries: list[BlameEntry]) -> dict[str, str]:
        ...
        authors = list({e.author for e in entries if e.author})
        colors = [
            "#e53e3e",
            "#dd6b20",
            "#d69e2e",
...
def compare_blame_authorship(
    file_a: str, file_b: str, repo_path: str
) -> dict[str, Any]:
    """
    ...
    set(authors_a.keys()) | set(authors_b.keys())
