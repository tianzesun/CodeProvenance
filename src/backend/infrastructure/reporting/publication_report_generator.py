...
class PublicationReportGenerator:
    ...
    def generate(self, case_data: Dict[str, Any], output_path: Path):
        ...
        context = {
            ...
            "report_hash": hashlib.sha256(
                json.dumps(case_data, sort_keys=True).encode()
            ).hexdigest()[:12],
        }
    ...
    def _get_interpretation(self, score: float) -> str:
        if score > 0.85:
            return "Critical overlap detected requiring immediate committee review. High probability of plagiarism."
        if score > 0.6:
            return "High similarity scores from multiple independent forensic engines. Manual review highly recommended."
            ...
            processed.append(
                {
                    ...
                    "risk": (
                        "critical"
                        if exp.get("score", 0) > 0.9
                        else ("high" if exp.get("score", 0) > 0.75 else "medium")
                    ),
                }
            )
        return processed
