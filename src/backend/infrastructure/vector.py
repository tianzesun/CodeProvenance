...
class VectorSearchManager:
    ...
    async def find_similar_to_file(
        self, file_id: str, tenant_id: str, top_k: int = 10, min_similarity: float = 0.7
    ) -> list[dict[str, Any]]:
        """
    ...
    async def get_embedding(self, file_id: str, job_id: str) -> CodeEmbedding | None:
        """
    ...
    def generate_code_embedding(self, code: str, language: str) -> list[float]:
        ...
        try:
            ...
            response = client.embeddings.create(
                input=enhanced_code[:8000], model=self.model  # Token limit
            )
...
class SemanticSearchService:
    ...
    def __init__(
        self, vector_manager: VectorSearchManager, embedding_provider: EmbeddingProvider
    ):
        """
    ...
    async def get_similarity_with_file(
        self, code: str, language: str, existing_file_id: str, tenant_id: str
    ) -> float | None:
        """
        ...
        if not existing:
            ...
            similar = await self.vector_manager.find_similar(
                query_vector=vector, tenant_id=tenant_id, top_k=1, exclude_file_ids=[]
            )
            if similar:
...
def create_semantic_search_service(
    db_pool=None, api_key: str | None = None
