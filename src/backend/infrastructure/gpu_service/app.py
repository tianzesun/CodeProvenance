...
from .schemas import BatchRequest, CompareRequest, TaskResponse
...
async def predict(req: CompareRequest):
    ...
    if model is None:
        raise HTTPException(
            status_code=503, detail=f"Model not available: {_model_error}"
        )
    ...
    risk = (
        "CRITICAL"
        if score >= 0.9
        else "HIGH" if score >= 0.75 else "MEDIUM" if score >= 0.5 else "LOW"
    )
...
async def predict_batch(req: BatchRequest):
    ...
    for s in scores:
        risk = (
            "CRITICAL"
            if s >= 0.9
            else "HIGH" if s >= 0.75 else "MEDIUM" if s >= 0.5 else "LOW"
        )
