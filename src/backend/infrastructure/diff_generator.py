...
class ASTAligner:
    ...
    def align(
        self, code_a: str, code_b: str, ast_a: Any = None, ast_b: Any = None
    ) -> list[MatchBlock]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if any(
                ...
                if current_indent > indent_stack[-1] or not stripped:
                    ...
                    if (
                        stripped
                        and current_indent <= indent_stack[-1]
    ...
    def align(self, code_a: str, code_b: str) -> list[MatchBlock]:
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == "equal" and (i2 - i1) >= self.min_block_size:
                # Find line numbers
                a_start = (
                    code_a[: sum(len(t) + 1 for t in tokens_a[:i1])].count("\n") + 1
                )
                a_end = code_a[: sum(len(t) + 1 for t in tokens_a[:i2])].count("\n") + 1
                b_start = (
                    code_b[: sum(len(t) + 1 for t in tokens_b[:j1])].count("\n") + 1
                )
...
class RegionMerger:
    ...
    def _overlaps(self, a: MatchBlock, b: MatchBlock) -> bool:
        ...
        return not (
            a.a_end < b.a_start
            or b.a_end < a.a_start
            or a.b_end < b.b_start
            or b.b_end < a.b_start
        )
