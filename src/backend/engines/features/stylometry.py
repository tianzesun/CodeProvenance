...
class StylometryExtractor:
    ...
    def _extract_ast_stylometry(self, tree: ast.AST) -> dict[str, Any]:
        ...
        features = {
            "avg_func_length": (
                np.mean(stats["func_lengths"]) if stats["func_lengths"] else 0
            ),
            "max_func_length": (
                np.max(stats["func_lengths"]) if stats["func_lengths"] else 0
            ),
        ...
        if stats["var_naming"]:
            ...
            is_camel = sum(
                1
                for v in stats["var_naming"]
                if any(c.isupper() for c in v) and "_" not in v
            )
