...
class EvidenceCompressor:
    ...
    def compress(
        ...
        for gst in gst_blocks:
            ...
            gst_strength = gst.get(
                "match_strength", gst.get("match_percent", 0.0) / 100.0
            )
    ...
    def _find_ast_support(
        ...
        for a_node, b_node in ast_matches:
            ...
            if (
                ("function" in node_type and "def" in snippet)
                or ("call" in node_type and "(" in snippet)
                or ("loop" in node_type and ("for" in snippet or "while" in snippet))
                or ("if" in node_type and "if" in snippet)
                or ("return" in node_type and "return" in snippet)
            ):
    ...
    def format_report(self, bundles: List[EvidenceBundle]) -> str:
        ...
        for i, bundle in enumerate(bundles, 1):
            ...
            lines.append(
                f"  Score: {bundle.score:.2f} | Confidence: {bundle.confidence:.1%}"
            )
            ...
            "evidence_bundles": [
                {
                    ...
                    "code_range": [
                        b.gst_block.get("a_start", 0),
                        b.gst_block.get("a_end", 0),
                    ],
