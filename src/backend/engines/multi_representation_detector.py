...
class MultiRepresentationDetector:
    ...
    def query(
        ...
        all_features = (
            query_features["graph"]["hashes"] | query_features["fingerprints"]
        )
        signature = self.minhash.hash_set(all_features)
        ...
        for cid in candidates:
            ...
            graph_sim = self._jaccard_set(
                query_features["graph"]["hashes"], candidate_features["graph"]["hashes"]
            )

            # 🔢 Step 3: Fingerprint similarity (fast structural match)
            fp_sim = self._jaccard_set(
                query_features["fingerprints"], candidate_features["fingerprints"]
            )
            ...
            if graph_sim > 0.4 or fp_sim > 0.4:
                seq_sim = self.gst_matcher.similarity(
                    query_features["sequence"], candidate_features["sequence"]
                )
            ...
            combined_score = 0.45 * graph_sim + 0.30 * fp_sim + 0.25 * seq_sim
...
class GraphFeatureExtractor:
    ...
    def extract(self, code: str) -> dict[str, Any]:
        ...
        return {"hashes": hashes, "cfg_edges": cfg_hash, "complexity": comp_hash}
        ...
        while True:
            ...
            for i in range(len(seq_a)):
                ...
                for j in range(len(seq_b)):
                    ...
                    while (
                        i + length < len(seq_a)
                        and j + length < len(seq_b)
                        and not marked_a[i + length]
                        and not marked_b[j + length]
                        and seq_a[i + length] == seq_b[j + length]
                    ):
                        length += 1
