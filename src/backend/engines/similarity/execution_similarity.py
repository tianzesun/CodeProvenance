...
class DockerSandbox:
    ...
    def __init__(
        self, timeout: int = 10, memory_limit: str = "256m", cpu_limit: float = 0.5
    ):
        """
    ...
    def _execute_local(
        ...
        if not interpreter:
            return ExecutionResult(
                stderr=f"Unsupported language: {language}", exit_code=-1
            )
...
class TestCaseGenerator:
    ...
    def _analyze_code(self, code: str) -> dict[str, Any]:
        ...
        for pattern in func_patterns:
            ...
            if functions:
                result["functions"] = [
                    f for f in functions if f not in ["main", "__init__"]
                ]
                if result["functions"]:
...
class ExecutionSimilarity(BaseSimilarityAlgorithm):
    ...
    def compare(self, parsed_a: dict[str, Any], parsed_b: dict[str, Any]) -> Finding:
        ...
        if not self._is_executable(code_a, language) or not self._is_executable(
            code_b, language
        ):
            return Finding(
                engine=self.name,
    ...
    def _outputs_match(
        self, result_a: ExecutionResult, result_b: ExecutionResult
    ) -> bool:
