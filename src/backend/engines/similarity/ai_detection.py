...
using a multi-signal ensemble that outperforms single-heuristic approaches.
...
_LLM_COMMENT_PATTERNS: List[re.Pattern] = [
    ...
    re.compile(
        r"#\s*[A-Z][a-z]+(?: [a-z]+){2,}\s*$", re.M
    ),  # Title-case full-sentence comments
    re.compile(r"#\s*-{3,}"),  # Separator comments like # ---
    re.compile(
...
_LLM_STRUCTURAL_PATTERNS: List[re.Pattern] = [
    re.compile(r"if\s+\w+\s+is\s+None\s*:", re.I),  # Explicit None checks
    re.compile(r"raise\s+ValueError\s*\(f?['\"]", re.I),  # Formatted ValueError raises
    re.compile(r"raise\s+TypeError\s*\(f?['\"]", re.I),
    re.compile(r"logging\.(debug|info|warning|error)\s*\(f?['\"]", re.I),
    re.compile(r"return\s+\[\s*\w+\s+for\s+\w+\s+in\s+\w+\s*\]"),  # List comp returns
    re.compile(r"Optional\["),  # Type hint heavy
    re.compile(r"Union\["),
...
class AIDetectionEngine:
    ...
    # These were estimated from a sample of open-source Python repos.
    _HUMAN_BIGRAM_FREQ: Dict[str, float] = {
        "if not": 0.031,
        "return none": 0.018,
        "self .": 0.045,
        "for i": 0.022,
    ...
    def analyze(self, code: str, language: str = "python") -> Dict[str, Any]:
        ...
        Returns:
            Dict with keys:
                ...
                signal_labels   Dict[str, str]    — human-readable labels
                indicators      List[str]         — top evidence strings
        ...
        try:
            ...
            # Add legacy alias so existing tests/code using "pattern_repetition" still work
    ...
    def _signal_perplexity(self, code: str) -> float:
        ...
        LLMs produce lower-entropy token streams than humans.
        We measure bigram entropy and invert it so high = AI-like.
        ...
        ai_score = max(0.0, min(1.0, 1.0 - combined / 5.0))
        return ai_score
    ...
    def _signal_burstiness(self, code: str) -> float:
        ...
        # Map cv=0.2 → 0.85 (AI), cv=1.2 → 0.10 (human)
        ai_score = max(0.0, min(1.0, 1.0 - (cv / 1.2)))
        return ai_score
    ...
    def _signal_stylometry(self, code: str) -> float:
        ...
        # High descriptive variable ratio → AI
        score += features.descriptive_var_ratio * 0.25
        checks += 0.25

        # High docstring ratio → AI
        score += features.docstring_ratio * 0.20
        checks += 0.20

        # High type hint ratio → AI (LLMs are pedantic about types)
        score += features.type_hint_ratio * 0.20
        checks += 0.20

        # Low single-char variable ratio → AI (humans use i, j, x, n)
        score += (1.0 - features.single_char_var_ratio) * 0.15
        checks += 0.15

        # High exception handling ratio → AI
        score += features.exception_handling_ratio * 0.10
        checks += 0.10

        # High list comprehension ratio → AI
    ...
    def _signal_pattern_library(self, code: str) -> float:
        ...
        # Normalise: 1 match per 10 lines → score ~0.5
    ...
    def _signal_structural_entropy(self, code: str, language: str) -> float:
        ...
        We measure the entropy of AST node types and invert it.
        """
        if language not in ("python", "py", ""):
            # For non-Python, fall back to indent-block uniformity
        ...
        # Typical human Python: entropy ~0.65; LLM Python: ~0.50
        ai_score = max(0.0, min(1.0, 1.0 - entropy))
        return ai_score * 0.8  # Dampen — this signal is noisy
    ...
    def _signal_vocabulary_richness(self, code: str) -> float:
        ...
        # Standardised TTR over windows of 50 tokens
        window = 50
        ttrs = []
        ...
        # High TTR and hapax → human; low → AI
        # Calibrated: human TTR ~0.55, LLM TTR ~0.40
    ...
    def _signal_whitespace_rhythm(self, code: str) -> float:
        ...
        LLMs produce very regular blank-line spacing (exactly one blank
        line between functions, exactly two before classes).
        We measure the entropy of blank-line run lengths.
        """
        ...
        # Low entropy (very regular spacing) → AI
        return max(0.0, min(1.0, 1.0 - entropy))

    def _signal_docstring_density(self, code: str) -> float:
        """Docstring density signal.

        LLMs add docstrings to almost every function, including trivial ones.
        We count the ratio of triple-quoted strings to function definitions.
        """
        func_count = len(
            re.findall(
        ...
        if func_count == 0:
            # No functions — check raw docstring density
            lines = code.splitlines()
            if not lines:
                return 0.0
        ...
        # Human code: ratio ~0.2–0.5; LLM code: ratio ~0.8–1.2
    ...
    def _fuse(self, signals: Dict[str, float]) -> float:
        """Weighted average fusion of all signals."""
        total_score = 0.0
        total_weight = 0.0
        ...
        # Calibration: apply a mild sigmoid stretch so the output
        # distribution is more spread out (avoids clustering at 0.4–0.5).
        # f(x) = 1 / (1 + exp(-k*(x - 0.5)))  with k=6
        k = 6.0
    ...
    def _confidence(self, signals: Dict[str, float], ai_prob: float) -> float:
        ...
        # Agreement: lower std_dev → higher agreement
    ...
    def _combine_signals(self, signals: Dict[str, float]) -> float:
        ...
        weights = {
            ...
            "stylometry": 0.16,
            "pattern_library": 0.20,
            "pattern_repetition": 0.20,  # old name alias
            "structural_entropy": 0.12,
            "vocabulary_richness": 0.08,
            "whitespace_rhythm": 0.06,
            "docstring_density": 0.06,
