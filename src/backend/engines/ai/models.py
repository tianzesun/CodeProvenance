...
class AIDetectionResult:
    ...
    def _validate_probabilities(self):
        ...
        if not (0.0 <= self.ai_probability <= 1.0):
            raise ValueError(f"ai_probability must be in [0.0, 1.0], got {self.ai_probability}")
    ...
    def _validate_indicators(self):
        ...
        if len(self.indicators) > 6:
            raise ValueError(f"indicators must have at most 6 items, got {len(self.indicators)}")
    ...
    def _validate_flagged_lines(self):
        ...
        if len(self.flagged_lines) > 30:
            raise ValueError(f"flagged_lines must have at most 30 items, got {len(self.flagged_lines)}")
        for line_num in self.flagged_lines:
            if not isinstance(line_num, int) or line_num < 1:
                raise ValueError(f"each flagged line must be a positive integer, got {line_num}")
