...
class SemanticLayer:
    ...
    MAX_CAP = 0.90  # Hard cap to prevent false positives
    ...
    def evaluate(self, embedding_score: float) -> LayerResult:
        ...
        # Hard cap
        capped = min(corrected, self.MAX_CAP)
        
        return LayerResult(
            score=capped,
            signals={"embedding": embedding_score, "embedding_corrected": capped},
            triggered=capped > 0.0
        )
