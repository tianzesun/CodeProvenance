...
class ImmutableSubmission:
    ...
    student_id: str | None = None  # May be redacted for committee view
...
class EvidenceBlock:
    ...
    a_snippet: str = ""
    b_snippet: str = ""
    transformation_notes: list[str] = field(
        default_factory=list
    )  # e.g., "variable renamed"
...
class AcademicIntegrityCase:
    ...
    submission_a: ImmutableSubmission | None = None
    submission_b: ImmutableSubmission | None = None
    ...
    reviewed_by: str | None = None  # Professor who reviewed
    committee_decision: str | None = None  # Committee outcome
    ...
    def clear(self, reviewer: str, rationale: str = "") -> None:
        ...
        self.reviewed_by = reviewer
        self.audit_log.append(
            AuditEntry(actor=reviewer, action="cleared", details=rationale)
        )
    ...
    def refer_to_committee(self, referrer: str, rationale: str = "") -> None:
        ...
        self.audit_log.append(
            AuditEntry(
                actor=referrer, action="referred_to_committee", details=rationale
            )
        )
    ...
    def appeal(self, student_id: str, rationale: str = "") -> None:
        ...
        self.audit_log.append(
            AuditEntry(actor=student_id, action="appealed", details=rationale)
        )
    ...
    def export_committee_file(self) -> dict[str, Any]:
        ...
        return {
            ...
            "submission_a": (
                self.submission_a.redact_for_committee().__dict__
                if self.submission_a
                else None
            ),
            "submission_b": (
                self.submission_b.redact_for_committee().__dict__
                if self.submission_b
                else None
            ),
            "findings": [
