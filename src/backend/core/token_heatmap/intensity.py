...
class HeatIntensityCalculator:
        ...
        if b.start < a.end:
            ...
            result.append(
                TokenSpan(
                    ...
                    confidence=max(a.confidence, b.confidence),
                    match_type=(
                        a.match_type if a.confidence >= b.confidence else b.match_type
                    ),
                    token_type=a.token_type or b.token_type,
                )
            )
    ...
    def build_heatmap_array(
        self, spans: list[TokenSpan], code_length: int
    ) -> list[float]:
    ...
    def get_coverage_stats(
        ...
        return {
            ...
            "avg_confidence": (
                sum(highlighted_confidences) / len(highlighted_confidences)
                if highlighted_confidences
                else 0.0
            ),
