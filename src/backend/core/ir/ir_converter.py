...
class IRConverter:
    ...
    def ast_to_graph(ast_ir: ASTIR) -> GraphIR:
        ...
        def process_node(ast_node: ASTNode, parent_id: str | None = None) -> str:
    ...
    def token_to_graph(token_ir: TokenIR) -> GraphIR:
        ...
        for i, token in enumerate(token_ir.tokens):
            ...
            if i > 0:
                edges.append(
                    GraphEdge(
                        source_id=f"token_{i-1}", target_id=node_id, edge_type="next"
                    )
                )
    ...
    def merge_graphs(graphs: list[GraphIR]) -> GraphIR:
        ...
        for graph in graphs:
            ...
            for edge in graph.edges:
                ...
                if edge_key not in {
                    (e.source_id, e.target_id, e.edge_type) for e in all_edges
                }:
