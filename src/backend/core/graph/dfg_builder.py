        ...
        for param in func_def.args.args:
            if param.arg != "self":
                df_node = self._create_df_node(
                    ...
                    cfg_node_id=self._find_cfg_node_for_line(
                        cfg, getattr(param, "lineno", 0)
                    ),
    ...
    def _handle_assign(
        ...
        for target in node.targets:
            if isinstance(target, ast.Name) and self._is_user_variable(target.id):
                ...
                if isinstance(target.value, ast.Name) and self._is_user_variable(
                    target.value.id
                ):
                    df_node = self._create_df_node(
    ...
    def _collect_uses(
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.Name):
                if (
                    self._is_user_variable(child.id)
                    and child.id not in self._global_names
                ):
        ...
        for scope, var_dict in by_scope_var.items():
            for var_name, state_dict in var_dict.items():
                ...
                for mod_node in mods:
                    for use_node in uses:
                        if mod_node.id != use_node.id and self._could_reach(
                            cfg, mod_node, use_node
                        ):
                            dfg.add_edge(
                                DFEdge(
                                    source=mod_node.id,
