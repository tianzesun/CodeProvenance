...
class RateLimiter:
    ...
    def check_rate_limit(self, tenant_id: str, request: Request) -> None:
        ...
        with self.lock:
            ...
            # Check minute limit
            minute_count = self._count_requests_in_window(tenant_id, now - 60)
            ...
            # Check hour limit
            hour_count = self._count_requests_in_window(tenant_id, now - 3600)
            ...
            # Check day limit
            day_count = self._count_requests_in_window(tenant_id, now - 86400)
    ...
    def _count_requests_in_window(self, tenant_id: str, since: float) -> int:
        ...
        return sum(1 for ts in self.request_counts[tenant_id] if ts > since)
