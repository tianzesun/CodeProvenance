...
async def get_job(request: Request, job_id: uuid.UUID, db: Session = Depends(get_db)):
    """
    Get a job by ID.
    """
    tenant_id = get_current_tenant(request)
...
async def list_jobs(
    ...
    jobs = JobService.get_jobs_by_tenant(
        db, tenant_id, status=status, limit=limit, offset=skip
    )
    return jobs
