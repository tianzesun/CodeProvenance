...
def _infer_dataset_size_label(
    ...
    unique_files = _count_unique_code_files(dataset_dir)
    if unique_files > 0:
        return f"{unique_files:,} files"
...
def _build_benchmark_dataset_readiness(
    ...
    if dataset_id in {"human_eval", "mbpp"}:
        runnable = _has_loadable_huggingface_dataset(dataset_root)
        return {
            "runnable": runnable,
            "status": "ready" if runnable else "incomplete_huggingface_dataset",
...
def _load_job_from_db(job_id: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.exception(f"_load_job_from_db failed for {job_id}")
        return None
...
def _build_ai_detection_summary(submissions: dict[str, str]) -> dict[str, Any]:
    ...
    for name, code in submissions.items():
        ...
        calibration_info = {
            "available": bool(result.get("calibration", {}).get("available")),
            "samples": int(result.get("calibration", {}).get("samples") or 0),
        }
    ...
    # Most indicative signal: the one with the highest average across files.
    highest_signal_name = (
        max(signal_summary, key=lambda name: signal_summary[name]["average"])
        if signal_summary
        else ""
    )
    _SIGNAL_LABELS = {
    ...
    highest_signal = (
        entries[0].get("signal_labels", {}).get(highest_signal_name)
        if highest_signal_name and entries
        else None
    ...
    return {
        ...
        "highest_signal_value": highest_signal_value,
        "calibration_confidence": (
            round(min(1.0, calibration_info["samples"] / 20.0), 3)
...
async def get_analytics_overview(request: Request) -> dict[str, Any]:
    ...
    with SessionLocal() as db:
        ...
        repeat_offenders = [
            {"label": label, "value": value} for label, value in repeat_buckets.items()
        ]
                ...
                if not db.query(Job).filter(Job.id == job_id).first():
                    ...
                    for sub_name in list(submissions.keys())[:100]:
                        ...
                        try:
                            ...
                            db.commit()
                        except SQLAlchemyError as sub_err:
                            db.rollback()
                            logger.warning(
                                "Submission DB insert skipped for %s/%s: %s",
                                job_id,
                                sub_name,
                                sub_err,
                            )
...
def _run_analysis_background(
    ...
    """Synchronous wrapper around the async ``_run_analysis`` coroutine.

    FastAPI ``BackgroundTasks`` execute in a thread pool, not on the async
    event loop, so they must be ordinary synchronous functions.  This wrapper
    creates a fresh event loop, runs the coroutine to completion, and then
    cleans up the loop regardless of success or failure.

    Args:
        job_id: The short job identifier (8-char prefix) already inserted into
            ``_jobs`` by the calling endpoint.
        job_dir: Path to the directory containing the uploaded submission files.
        ...
        current_user: Authenticated user dict or ``None`` for guest uploads.
        engine_keys_raw: JSON-encoded list of engine keys to enable.
        tool_ids_raw: Comma-separated list of external tool IDs.
        starter_sources: Optional list of starter/template source strings.
    """
    import asyncio
    ...
    try:
        ...
        if job_id in _jobs:
            ...
            try:
                ...
                logger.warning("Could not persist failed job state for %s", job_id)
    finally:
        loop.close()
...
USER_EDITABLE_SETTINGS_DEFAULTS: dict[str, Any] = {
    ...
    "v1_review_rules": {
        ...
        "minimum_agreeing_methods": 2,
        "auto_flag_critical_matches": True,
        "short_match_suppression": True,
        "default_review_state": "needs_review",
        "manual_review_for_critical": True,
        "second_review_required": True,
        "second_review_threshold": 0.85,
        "allow_reviewer_dismissal": True,
        "require_dismissal_note": True,
        "show_side_by_side": True,
        "show_explanation_summary": True,
        "show_method_contributions": True,
        "show_confidence_labels": True,
        "show_transformed_match_notes": True,
        "report_sort_order": "risk_desc",
        "hide_low_confidence_by_default": True,
        "downloadable_reports_enabled": True,
        "include_reviewer_notes_in_exports": True,
    },
    "v1_ai_models": {
        "openai_enabled": True,
        ...
        "provider_status_alerts": True,
        "explanation_model": settings.OPENAI_MODEL,
        "cross_language_model": settings.OPENAI_MODEL,
        "model_version_pinning": True,
        "minimum_ai_confidence": 0.60,
        "minimum_explanation_confidence": 0.65,
        "require_deterministic_support": True,
        "semantic_similarity_enabled": True,
        "ai_explanations_enabled": True,
        "cross_language_analysis_enabled": False,
        "ai_influences_final_score": True,
        "explanation_verbosity": "standard",
        "max_ai_requests_per_check": 20,
        "max_tokens_per_request": 4000,
        "ai_timeout_seconds": 30,
        "disable_ai_when_limits_reached": True,
    },
    "v1_detection_methods": {
        "apply_all_enabled_by_default": True,
        "method_selection_by_language": False,
        "disable_unsupported_methods_automatically": True,
        "minimum_token_match_length": 8,
        "ngram_size": 5,
        "winnowing_window_size": 4,
        "minimum_ast_similarity": 0.30,
        "minimum_embedding_similarity": 0.55,
    },
    "v1_scoring": {
        "active_formula": "weighted_v1",
        "formula_version": "v1",
        "weighted_score_calculation": True,
        "normalize_by_file_size": True,
        "penalize_short_matches": True,
        "boost_cross_method_agreement": True,
        "suspicion_threshold": 0.45,
        "review_threshold": settings.DEFAULT_THRESHOLD,
        "critical_threshold": 0.85,
        "short_match_threshold": 0.30,
        "ai_only_threshold": 0.75,
        "ignore_threshold": 0.25,
        "high_risk_threshold": 0.75,
        "require_second_review_above_critical": True,
        "require_non_ai_confirmation_below_ai_only": True,
        "auto_hide_ignored_matches": True,
        "auto_prioritize_high_risk_matches": True,
        "active_profile": "balanced",
        "custom_profile_name": "",
        "calibration_mode_enabled": False,
        "compare_current_vs_previous": True,
        "save_manual_adjustments_as_version": True,
        "require_approval_before_production_update": True,
        "calibration_notes": "",
        "primary_evaluation_metric": "f1",
    ...
    "v1_system_limits": {
        ...
        "allowed_file_types": ".py,.java,.js,.ts,.cpp,.c,.cs,.go,.rs,.zip",
        "max_submissions_per_batch": 500,
        "max_check_runtime_seconds": 900,
        "max_method_runtime_seconds": 120,
        "ai_timeout_per_task_seconds": 30,
        ...
        "temporary_upload_retention_days": 7,
        "audit_log_retention_days": 365,
        "automatic_cleanup_schedule": "daily",
    },
}
