...
class StudentFeedbackService:
    ...
    def __init__(
        self, analyzer: SubmissionAnalyzer, behavioral_service: KeystrokeAnalysisService
    ):
        self.analyzer = analyzer
        self.behavioral_service = behavioral_service

    def provide_pre_submission_feedback(
        self, code: str, keystrokes: list[Any] | None = None
    ) -> dict[str, Any]:
        """
        ...
        if risk_level in ["CRITICAL", "HIGH"]:
            suggestions.append(
                "Your code has a high level of similarity to existing sources. Please ensure you are citing your references or rewrite this section in your own words."
            )

        if behavioral_results.get("is_suspicious"):
            suggestions.append(
                "We detected large blocks of text being pasted. Remember that copying and pasting code without attribution is considered a breach of academic integrity."
            )

        if results.get("ai_probability", 0.0) > 0.7:
            suggestions.append(
                "The structure of your code is highly similar to AI-generated patterns. We recommend reviewing this section to ensure it reflects your personal coding style."
            )
